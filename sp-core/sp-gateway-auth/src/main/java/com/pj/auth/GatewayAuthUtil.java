...
public class GatewayAuthUtil {
	...
	public static String getToken() {
		String currentToken = RedisUtil.get(VAR_GATEWAY_AUTH_TOKEN_KEY);
		if(isEmpty(currentToken)) {
			refreshToken();
			currentToken = RedisUtil.get(VAR_GATEWAY_AUTH_TOKEN_KEY);
		}
		return currentToken;
	}
	...
	public static String getLowToken() {
		return RedisUtil.get(VAR_GATEWAY_AUTH_LOW_TOKEN_KEY);
	}
	...
	public static boolean verifyToken(String token) {
		...
		String currentTokenLow = RedisUtil.get(VAR_GATEWAY_AUTH_LOW_TOKEN_KEY);
	...
	public static void refreshToken() {
		...
		if(isEmpty(currentToken) == false) {
			RedisUtil.set(VAR_GATEWAY_AUTH_LOW_TOKEN_KEY, currentToken);
		}
		// s2. 再刷新当前token 
		RedisUtil.set(VAR_GATEWAY_AUTH_TOKEN_KEY, createToken());
		RedisUtil.set(VAR_GATEWAY_AUTH_TOKEN_REFRESH_LAST_TIME_KEY, String.valueOf(System.currentTimeMillis()));
	}
